...
void update_and_render(Player *p, const int32_t w, const int32_t h)
{
    ...
    for (int32_t x = 0; x < w; x++) {
        ...
        // Calculate distance the ray travels between grid lines
        ...
        // Perform Digital Differential Analysis (DDA) to find wall collision
        while (hit == 0) {
        ...
        // Calculate distance projected on camera direction to eliminate fisheye distortion
        if (side == 0) {
        ...
        // Calculate height of the vertical line to draw on screen
        ...
        // Render the vertical column slice
