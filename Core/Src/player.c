...
{
    // Set initial position and orientation vectors in Q16.16 fixed-point format
...
{
    // Calculate movement step along the view direction
    ...
    // Check wall collision independently on X and Y axes for smooth sliding behavior
...
{
    // Calculate movement step opposite to the view direction
    int32_t step_x = FIXED_PRODUCT(p->direction_x, speed);
    int32_t step_y = FIXED_PRODUCT(p->direction_y, speed);
...
{
    // Retrieve sine and cosine values for the rotation angle
    int32_t cos_val = lookup_cos(angle_delta);
    int32_t sin_val = lookup_sin(angle_delta);

    // Rotate the view direction vector using a 2D rotation matrix
    ...
    // Rotate the camera plane vector to maintain the correct field of view
